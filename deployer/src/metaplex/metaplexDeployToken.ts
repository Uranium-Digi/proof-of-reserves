...
export const createAndMintTokensViaMetaplex = async (
    ...
    const tokenConfig = createTokenConfig(name, symbol, description, imageUri, initialSupply, vanityAddress)
    ...
    // Creating the mintIx
    const mintSigner: KeypairSigner = tokenConfig.vanityAddress ? tokenConfig.vanityAddress : generateSigner(umi)
